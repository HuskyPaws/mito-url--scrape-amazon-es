...
# Streamlit app title
st.title("Amazon Product Data Scraper - ES")
...
def scrape_data(url, api_key, max_retries=5, initial_delay=2):
    ...
    while retries < max_retries:
        try:
            ...
            if byline_info:
                data_dict["Brand Store URL"] = "https://www.amazon.es" + byline_info[0].get('href')
                ...
                for element in response_json.get('data', []):
                    ...
                    # Handle detail bullets
                        ...
                        if not element.get('error') and element.get('results'):
                            ...
                            # Check for manufacturer/brand with Spanish terms
                            if "Marca" in bullet_points:
                                try:
                                    manufacturer = bullet_points.split("Marca")[1].strip().split("\n")[0].strip()
                                    data_dict["Manufacturer"] = manufacturer.split(":")[1].strip() if ":" in manufacturer else manufacturer.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing Marca: {str(e)}")

                            elif "Fabricante" in bullet_points:
                                try:
                                    manufacturer = bullet_points.split("Fabricante")[1].strip().split("\n")[0].strip()
                                    data_dict["Manufacturer"] = manufacturer.split(":")[1].strip() if ":" in manufacturer else manufacturer.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing Fabricante: {str(e)}")

                            # Check for model number with Spanish terms
                            if "Número de modelo del producto" in bullet_points:
                                try:
                                    model_number = bullet_points.split("Número de modelo del producto")[1].strip().split("\n")[0].strip()
                                ...
                                try:
                                    ...
                                    logging.error(f"Error processing Product Number: {str(e)}")
                    ...
                    # Handle tech specs table
                        ...
                        if element.get('results'):
                            ...
                            if "Marca" in table_data and data_dict["Manufacturer"] == "Not found":
                                try:
                                    manufacturer = table_data.split("Marca")[1].strip().split("\n")[0].strip()
                                    data_dict["Manufacturer"] = manufacturer.split("\t")[1].strip() if "\t" in manufacturer else manufacturer.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing table Marca: {str(e)}")
                                ...
                                try:
                                    ...
                                    logging.error(f"Error processing table Fabricante: {str(e)}")
                            ...
                            if "Nombre del modelo" in table_data and data_dict["Item Model Number"] == "Not found":
                                try:
                                    ...
                                    logging.error(f"Error processing table Model Name: {str(e)}")

                            elif "Número de modelo del producto" in table_data and data_dict["Item Model Number"] == "Not found":
                                try:
                                    model_number = table_data.split("Número de modelo del producto")[1].strip().split("\n")[0].strip()
                                    data_dict["Item Model Number"] = model_number.split("\t")[1].strip() if "\t" in model_number else model_number.strip()
                                except (IndexError, KeyError) as e:
                                ...
                                try:
                                    ...
                                    logging.error(f"Error processing table Product Number: {str(e)}")
